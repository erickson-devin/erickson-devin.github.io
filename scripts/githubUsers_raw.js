function stringify() {
  ...
  },
  {
    ...
    "site_admin": false
            ...
            var stringifyExplain = ("<p><h3>Stringify was used to convert the above JSON data to this string:</h3></p>");
            var i = document.getElementById("showData").value;
            var userdata = '';
            		...
            		if(this.status == 200) {
            			...
            			var output, j = '';
            				for(j in userdata) {
            					output +=
            					'<div class="user">' +
            					'<img src="'+userdata[j].avatar_url+'" width="100" height="100"><br>' +
            					'ID: '+userdata[j].id+'<br>'+
            					'Username: '+userdata[j].login+'<br>'+
            					'<br></div>';
            				}
            				document.getElementById('parseData').innerHTML = output;
            			}
            		}
            		users.send();
            	
            var string = JSON.stringify(stringData);
